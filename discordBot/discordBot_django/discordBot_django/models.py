...
class Reminder(models.Model):
    ...
    reminder_date = PSTDateTimeField()
    original_message_channel_id = models.CharField(
        max_length=3000
    )
    ...
    def get_countdown(self, current_time):
        seconds = round(self.reminder_date.timestamp() - current_time.timestamp())
        day = seconds // (24 * 3600)
    ...
    def get_expired_reminders(cls):
        return list(Reminder.objects.all().filter(reminder_date__lte=pstdatetime.now().pst))
